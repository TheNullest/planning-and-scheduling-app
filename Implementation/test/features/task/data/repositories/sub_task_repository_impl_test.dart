...
void main() {
  ...
  group('getSubTasksByStatus', () {
    test(
        ...
        'which will turn into [Right(List<SubTaskEntity>)] data', () async {
      when(() => mockDataSource.getBatchByStatus(TaskStatus.inProgress))
          .thenAnswer((_) async => const Right([]));

      final result = await subTaskRepo.getSubTasksByStatus(TaskStatus.inProgress);

      expect(result.isRight(), true);
      expect(result, isA<Right<Failure, List<SubTaskEntity>>>());
      verify(() => mockDataSource.getBatchByStatus(TaskStatus.inProgress))
          .called(1); // Verify that get was only called once
      verifyNoMoreInteractions(mockDataSource);
    });
