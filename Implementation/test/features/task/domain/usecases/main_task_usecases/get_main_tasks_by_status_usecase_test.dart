...
import 'package:zamaan/features/tasks_management/domain/useCases/task/get_tasks_by_status_use_case.dart';
...
void main() {
  ...
  test(
      ...
      () async {
    //Arrange
    when(() => repository.getMainTasksByStatus(TaskStatus.completed))
        .thenAnswer((_) async => const Right([]));

    // Act
    final result = await useCase(TaskStatus.completed);

    // Assert
    expect(result.isRight(), true);

    expect(result, equals(const Right<dynamic, List<MainTaskEntity>>([])));
    verify(() => repository.getMainTasksByStatus(TaskStatus.completed)).called(1);
