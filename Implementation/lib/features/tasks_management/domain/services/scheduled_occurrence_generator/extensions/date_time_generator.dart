...
extension DateTimeGenerator on ScheduledOccurrenceGenerator {
  ...
  void _generateDates({
    ...
    while (compareDates(past: currentDate, future: scheduleConstraint.endDate) <= 0 &&
  ...
  /// Checks if a given [DateRangeEntity] overlaps with any pre-existing scheduled occurrence.
  ///
  /// Returns:
  /// - `true` if there is any interference; otherwise, `false`.
  bool _isThereInterference(DateRangeEntity date) {
    return scheduledOccurrences.any(
      (instance) =>
          instance.startDateTime.overlapsWith(startDate: date.startDate!, endDate: date.endDate!) ||
          instance.endDateTime.overlapsWith(startDate: date.startDate!, endDate: date.endDate!),
    );
  }
